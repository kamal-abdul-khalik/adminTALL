    ...
    <x-2col>
        <x-slot name="left">
            ...
            @if (auth()->user()->two_fa_active == false)
                ...
                <p>{{ __('Passwords can get stolen – especially if you use the same password for multiple sites.') }}
                    {{ __('Adding Two-Step Verification means that even if your password gets stolen, your account will remain secure.') }}
                </p>

                <p><b>{{ __('How does it work?') }}</b></p>

                <p>{{ __('After you turn on Two-Step Verification for your account, signing in will be a little different: You will enter your password, as usual.') }}
                </p>
        ...
        <x-slot name="right">
            ...
            <div class="card">
                ...
                @if (auth()->user()->two_fa_active == true && auth()->user()->two_fa_secret_key != null)
                ...
                @else
                    <p>{{ __('Authenticator apps generate random codes that you can use to sign in. They do not have access to your password or account information.') }}
                    </p>
                    <p>{{ __('1Password is a good authenticator app as is Authy.') }}</p>

                    <p><img src='{{ $inlineUrl }}' alt='code'></p>

                    <p>{{ __('Scan bar code in your authenticator app or manually enter this key') }}
                        {{ $secretKey }}</p>
